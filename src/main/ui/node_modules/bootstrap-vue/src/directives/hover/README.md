...
- A function reference that will be called whenever hover state changes. The callback is passed a
  single boolean argument. `true` indicates that the element (or component) is hovered by the customers
...
Hover state should not be used to convey special meaning, as screen reader customers and keyboard only
customers typically can not trigger hover state on elements.
