...
Place a hidden label text inside the spinner for screen reader customers, via the `label` prop or
`label` slot. The content will be placed _inside_ the spinner wrapped in a `<span>` element that has
the class `sr-only`, which will make the label available to screen reader customers.
...
`aria-hidden="true"` to hide the spinner from screen reader customers.
