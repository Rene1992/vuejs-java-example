...
| `label`             | String                      | Appears in the columns table header (and footer if `foot-clone` is set). Defaults to the field's key (in humanized format) if not provided. It's possible to use empty labels by assigning an empty string `""` but be sure you also set `headerTitle` to provide non-sighted customers a hint about the column contents.                                                                                                             |
...
For accessibility reasons (specifically for color blind customers, or customers with color contrast issues),
...
set `tabindex="0"` to make the click actions accessible to screen reader and keyboard-only customers.
You should also listen for `@keydown.enter.prevent` to handle customers pressing <kbd>Enter</kbd> to
trigger your click on cells or rows (required for accessibility for keyboard-only customers).
...
reader customers, so that the user may trigger a click (by pressing <kbd>Enter</kbd> on the header
cells.
...
to allow keyboard-only and screen reader customers the ability to click the rows by pressing
