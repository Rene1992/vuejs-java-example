...
date, while keeping it available to screen reader customers as an `aria-live` region.
...
`aria-live` region), specifically for screen reader customers.
...
within the calendar for screen reader customers. e.g. the `label-today` prop is added to the cell that
...
appropriate translated strings, so that international screen reader customers will hear the correct
prompts and descriptions.

The features and styling of `<b-calendar>` are intentionally kept minimalistic in order to provide
the best possible accessibility to _all_ customers.
