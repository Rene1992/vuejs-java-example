...
including `./views/user/show.ejs` from `./views/customers.ejs` you would
...
```javascript
let ejs = require('ejs'),
    customers = ['geddy', 'neil', 'alex'];

// Just one template
ejs.render('<?= customers.join(" | "); ?>', {customers: customers}, {delimiter: '?'});
// => 'geddy | neil | alex'

// Or globally
ejs.delimiter = '$';
ejs.render('<$= customers.join(" | "); $>', {customers: customers});
// => 'geddy | neil | alex'
```
 ...
 * Erubis, an ERB implementation which also runs JavaScript: http://www.kuwata-lab.com/erubis/customers-guide.04.html#lang-javascript
